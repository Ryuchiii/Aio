...
cd /usr/bin
#wget https://aio.tekirovpn.my.id/レスキセティワン/bot.zip
#unzip bot.zip
#mv bot/* /usr/bin
#chmod +x /usr/bin/*
#rm -rf bot.zip
clear
wget https://aio.tekirovpn.my.id/レスキセティワン/kyt.zip
unzip kyt.zip
...
[Unit]
Description=Simple lynz - @zheevpn
...
systemctl start lynz
systemctl enable lynz
systemctl restart lynz
cd /root

bott=$(systemctl status lynz | grep running | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
# STATUS SERVICE BOT
